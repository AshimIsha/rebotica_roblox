...
--создадим переменные, для LobbySpawn и RaceSpawn
local lobby = game.Workspace.LobbySpawn
local race = game.Workspace.RaceSpawn


--создадим переменную для хранения минимального числа игроков
...
--Teleport
--создадим функцию для отправки персонажа из лобби
function teleport(location)
	--в цикле ищем игроков
	for _, player in pairs(game.Players:GetChildren()) do 
		--пока нет игроков, ждать
		while player.Character == nil do 
			wait()
		end
		--создаем переменные для дальнейшего перемещения персонажа
...
while true do
	...
	--создадим цикл ожидания игроков
	while game.Players.NumPlayers < minimum_players do
		wait()
	end
